...
import { t as require_react } from "./react-BcHHwdAL.js";
import { t as require_react_dom } from "./react-dom-raHgs34l.js";
...
function createRouter(init) {
	...
	async function handleAction(request, location, submission, matches, opts) {
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) result = {
			...
			result = await callLoaderOrAction("action", request, actionMatch, matches, manifest, mapRouteProperties$1, basename, future.v7_relativeSplatPath);
	...
	async function handleLoaders(request, location, matches, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionData, pendingError) {
		...
		if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
			...
			}, pendingActionData ? { actionData: pendingActionData } : {}, updatedFetchers$1 ? { fetchers: new Map(state.fetchers) } : {}), { flushSync });
			return { shortCircuited: true };
		}
		...
		let redirect$1 = findRedirect(results);
		if (redirect$1) {
			if (redirect$1.idx >= matchesToLoad.length) {
				let fetcherKey = revalidatingFetchers[redirect$1.idx - matchesToLoad.length].key;
				fetchRedirectIds.add(fetcherKey);
			}
			await startRedirectNavigation(state, redirect$1.result, { replace });
			return { shortCircuited: true };
		}
	...
	async function handleFetcherAction(key, routeId, path, match, requestMatches, flushSync, submission) {
		...
		let actionResult = await callLoaderOrAction("action", fetchRequest, match, requestMatches, manifest, mapRouteProperties$1, basename, future.v7_relativeSplatPath);
		...
		if (redirect$1) {
			...
			return startRedirectNavigation(state, redirect$1.result);
		}
	...
	async function handleFetcherLoader(key, routeId, path, match, matches, flushSync, submission) {
		...
		fetchControllers.set(key, abortController);
		let originatingLoadId = incrementingLoadId;
		let result = await callLoaderOrAction("loader", fetchRequest, match, matches, manifest, mapRouteProperties$1, basename, future.v7_relativeSplatPath);
	...
	*/
	async function startRedirectNavigation(state$1, redirect$1, _temp2) {
		let { submission, fetcherSubmission, replace } = _temp2 === void 0 ? {} : _temp2;
		if (redirect$1.revalidate) isRevalidationRequired = true;
		let redirectLocation = createLocation(state$1.location, redirect$1.location, { _isRedirect: true });
		invariant(redirectLocation, "Expected a location on the redirect navigation");
		if (isBrowser$1) {
			let isDocumentReload = false;
			if (redirect$1.reloadDocument) isDocumentReload = true;
			else if (ABSOLUTE_URL_REGEX$1.test(redirect$1.location)) {
				const url = init.history.createURL(redirect$1.location);
				isDocumentReload = url.origin !== routerWindow.location.origin || stripBasename(url.pathname, basename) == null;
			}
			if (isDocumentReload) {
				if (replace) routerWindow.location.replace(redirect$1.location);
				else routerWindow.location.assign(redirect$1.location);
				return;
			}
		}
		pendingNavigationController = null;
		let redirectHistoryAction = replace === true ? Action.Replace : Action.Push;
		let { formMethod, formAction, formEncType } = state$1.navigation;
		if (!submission && !fetcherSubmission && formMethod && formAction && formEncType) submission = getSubmissionFromNavigation(state$1.navigation);
		let activeSubmission = submission || fetcherSubmission;
		if (redirectPreserveMethodStatusCodes.has(redirect$1.status) && activeSubmission && isMutationMethod(activeSubmission.formMethod)) await startNavigation(redirectHistoryAction, redirectLocation, {
			submission: _extends$2({}, activeSubmission, { formAction: redirect$1.location }),
	...
	async function callLoadersAndMaybeResolveData(currentMatches, matches, matchesToLoad, fetchersToLoad, request) {
		let results = await Promise.all([...matchesToLoad.map((match) => callLoaderOrAction("loader", request, match, matches, manifest, mapRouteProperties$1, basename, future.v7_relativeSplatPath)), ...fetchersToLoad.map((f) => {
			if (f.matches && f.match && f.controller) return callLoaderOrAction("loader", createClientSideRequest(init.history, f.path, f.controller.signal), f.match, f.matches, manifest, mapRouteProperties$1, basename, future.v7_relativeSplatPath);
			else return {
				type: ResultType.error,
...
async function callLoaderOrAction(type, request, match, matches, manifest, mapRouteProperties$1, basename, v7_relativeSplatPath, opts) {
	if (opts === void 0) opts = {};
	let resultType;
	let result;
	let onReject;
	...
	try {
		...
		if (match.route.lazy) if (handler) {
			...
			let values = await Promise.all([runHandler(handler).catch((e) => {
				handlerError = e;
			}), loadLazyRouteModule(match.route, mapRouteProperties$1, manifest)]);
			if (handlerError) throw handlerError;
			result = values[0];
		} else {
			await loadLazyRouteModule(match.route, mapRouteProperties$1, manifest);
...
async function resolveDeferredResults(currentMatches, matchesToLoad, results, signals, isFetcher, currentLoaderData) {
	for (let index = 0; index < results.length; index++) {
		...
		if (isDeferredResult(result) && (isFetcher || isRevalidatingLoader)) {
			...
			await resolveDeferredData(result, signal, isFetcher).then((result$1) => {
				if (result$1) results[index] = result$1 || results[index];
			});
		}
	}
}
