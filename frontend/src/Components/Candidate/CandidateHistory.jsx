...
import { Filter, Eye, Award, FileText, X, ChevronLeft, ChevronRight } from "lucide-react";
...
const CandidateHistory = () => {
  ...
  let historyInterviews = Array.isArray(interviews)
    ? interviews.filter(
        (i) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#f7f9fb] p-4 rounded-xl border border-[#e0e3e5]">
        <div>
          <h2 className="text-base font-extrabold text-[#070235]">Past Interview History</h2>
          <p className="text-xs text-[#47464f]">Review completed session reports and ratings</p>
        </div>
      ...
      {paginatedData.length === 0 ? (
        <div className="text-center py-12 bg-[#f7f9fb] rounded-xl border border-dashed border-[#c8c5d0]">
          ...
          <p className="text-xs font-semibold text-[#191c1e]">No completed interview history records</p>
        </div>
      ) : (
          ...
          <div className="w-full max-w-xl bg-white border border-[#e0e3e5] rounded-2xl shadow-2xl overflow-hidden transition-all">
            <div className="px-6 py-4 bg-[#070235] text-white flex items-center justify-between border-b border-[#1e1b4b]">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-sm">Evaluation Report Feedback</h3>
            ...
            <div className="p-6 space-y-4 text-left text-xs">
              <div className="grid grid-cols-2 gap-4 bg-[#f7f9fb] p-4 rounded-xl border border-[#e0e3e5]">
                <div>
                  <span className="text-[10px] font-mono text-[#787680] uppercase block mb-1">Status</span>
                  <span className="font-bold text-sm text-[#0058be]">{selectedFeedback.recommendation || "Completed Evaluation"}</span>
                </div>
                <div>
                  <span className="text-[10px] font-mono text-[#787680] uppercase block mb-1">Technical Score</span>
                  <span className="font-bold text-sm text-[#070235]">{selectedFeedback.rating || 4.5} / 5.0</span>
                </div>
              </div>

              <div className="space-y-1.5">
                <span className="font-mono font-semibold text-[#191c1e] text-[11px] block">Recruiter Feedback Notes</span>
                <p className="bg-[#f2f4f6] p-3 rounded-lg border border-[#e0e3e5] text-[#47464f] leading-relaxed">
                  {selectedFeedback.comments || selectedFeedback.feedback || "Detailed evaluation report submitted by interviewer."}
                </p>
              </div>
            </div>
